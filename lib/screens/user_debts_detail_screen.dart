...
class _UserDebtsDetailScreenState extends State<UserDebtsDetailScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<DebtProvider>(
        context,
        listen: false,
      ).fetchDebtsWithUser(widget.otherUserId);
    });
  }
  ...
  void inviteFriend() {
    const inviteLink = "https://dubeapp.com/download";
    final message =
        "Hey! I'm using Dubé to track debts. "
        "Download it here: $inviteLink";

    SharePlus.instance.share(
      ShareParams(text: message, subject: "Join me on Dubé"),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < debtProvider.debtsWithUser!.length; i++) {
      if (debtProvider.debtsWithUser?[i].borrowerId == widget.otherUserId) {
        totalCreditAmount +=
            debtProvider.debtsWithUser?[i].outstandingAmount ?? 0;
      } else {
        totalBorrowAmount +=
            debtProvider.debtsWithUser?[i].outstandingAmount ?? 0;
      }
    ...
    return Scaffold(
      appBar: AppBar(
        title: GestureDetector(
          ...
          child: Column(
            ...
            children: [
              Text(
                widget.otherUserName ?? 'Debt Details',
      ...
      body: debtProvider.isLoadingDebtsWithUser
          ...
          : debtProvider.debtsWithUser == null ||
                debtProvider.debtsWithUser!.isEmpty
          ? RefreshIndicator(
              onRefresh: _refreshDebts,
              child: Center(
                child: Column(
                  ...
                  children: [
                    Text(
                      'No debt threads found with ${widget.otherUserName ?? "this user"}.',
                    ),
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  if (kDebugMode) {
                    ...
                    print(
                      debtProvider.debtsWithUser!.map((d) => d.id).toList(),
                    );
                  }
                  ...
                  if (debt.askedToPay) {
                    if (isBorrower) {
                      payRequestMessage =
                          'You have been asked to make a payment';
                    } else {
                      payRequestMessage = 'Payment request sent';
                    }
                  }
                  // Determine direction from the perspective of the *current user*
                  final bool isCreditorForThisDebtThread =
                      debt.creditorId == debtProvider.currentUserId;
                  Color amountColor = isCreditorForThisDebtThread
                  ...
                  if (debt.outstandingAmount == 0) {
                    ...
                    debtAmount =
                        '$amountPrefix${currencyFormatter.format((debt.outstandingAmount ?? 0.0).abs())}';
                  }
